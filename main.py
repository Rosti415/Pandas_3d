...
class GameController(Entity):# клас для всього руху гри(контролю або налаштування)
    ...
    def input (self, key):
        ...
        if key == 'a' or key == 'd' or key == 'w' or key =='s':
            footstep_sound.play()
        if key == 'escape' and len(self.blocks) != 0: #якщо немає блоків в грі (не полчась),то не можна вимкнути меню
            self.toggle_pause()
    ...
    def load(self):
        ...
        with open("save.data", "rb") as file:#видаляємо старі блоки і додаєм нові і додаєм їх в список
            k = pickle.load(file)
            for i in range(k):
